...
static void *_job_state_dup2(gres_job_state_t *gres_js, int node_index)
{
	...
	if (gres_js->gres_per_bit_alloc && gres_js->gres_bit_alloc) {
		...
		new_gres_js->gres_per_bit_alloc[0] = xcalloc(
			bit_size(gres_js->gres_bit_alloc[0]), sizeof(uint64_t));
		memcpy(new_gres_js->gres_per_bit_alloc[0],
		       gres_js->gres_per_bit_alloc[0],
		       bit_size(gres_js->gres_bit_alloc[0]) * sizeof(uint64_t));
	}
 ...
 * assigned to this task. Prefer gres with cpu affinity that match the task
					   ...
					   int local_proc_id)
{
	...
	for (int i = 0; i <= local_proc_id; i++) {
		...
		/*
		 * First: Try to select a single device with cpu affinity
		...
		/*
		* Second: Try to select devices with cpu affinity
		...
		*/
		...
		if (gres_needed)
			_filter_shared_gres_per_task(gres_bit_alloc,
						     ...
						     &gres_needed, true,
						     (i == local_proc_id));
		/* Fourth: Select devices with any available shared gres */
		if (gres_needed)
			_filter_shared_gres_per_task(gres_bit_alloc,
						     usable_gres,
						     gres_per_bit_avail,
