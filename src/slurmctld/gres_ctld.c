...
static int get_sharing_cnt_from_shared_cnt(gres_job_state_t *gres_js,
...
static int cnt_topo_gres(gres_job_state_t *gres_js, int n,
...
static void copy_matching_gres_per_bit(gres_job_state_t *gres_js,
		      ...
		      bool new_alloc)
{
	...
	if (gres_js->gres_bit_alloc[node_offset]) {
		...
		if (gres_js->gres_per_bit_select &&
		    gres_js->gres_per_bit_select[node_offset]){
			if (!gres_js->gres_per_bit_alloc) {
			...
			memcpy(gres_js->gres_per_bit_alloc[node_offset],
			       gres_js->gres_per_bit_select[node_offset],
			       bit_size(gres_js->gres_bit_select[node_offset]) *
				       sizeof(uint64_t));
		}
		gres_js->gres_cnt_node_alloc[node_offset] =
			gres_js->gres_cnt_node_select[node_index];
	...
	if (gres_js->gres_bit_alloc[node_offset] &&
		...
		for (i = 0; i < gres_ns->topo_cnt; i++) {
			...
			gres_cnt = cnt_topo_gres(gres_js, node_offset,
	...
	for (j = 0; j < gres_ns->type_cnt; j++) {
		...
		if (gres_js->gres_bit_alloc &&
		    gres_js->gres_bit_alloc[node_offset]) {
			if (shared_gres)
				gres_cnt = get_sharing_cnt_from_shared_cnt(
		...
		if (gres_js->gres_per_bit_alloc &&
		    gres_js->gres_per_bit_alloc[node_offset]) {
			copy_matching_gres_per_bit(gres_js, gres_js_alloc,
						   node_offset);
		}
	}
				    ...
				    gres_step_state_t *gres_ss, int n)
{
	...
	if (!gres_js->gres_per_bit_step_alloc[n])
				    ...
				    bitstr_t *core_bitmap)
{
	...
	if (gres_id_shared(gres_state_job->config_flags) && decr_job_alloc) {
